...
import FirebaseModelUtils from "../constants/Firebase";
...
export default class NetworkManager {
  ...
  static getUserByUuid = async (userUuid: string): Promise<User | undefined> => {
    console.info("------starting getUserByUuid-------")
    const db = firebase.firestore();
    const document = await db.collection(Collections.USERS)
      .doc(userUuid)
      .get();
    let jona = {
      db: db,
